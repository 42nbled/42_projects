...
static void parse_table_64(t_lst *root, t_Elf64_Sym *symbol_table, char *stringtable, size_t size, char *shstrtab, t_secinfo64 *sections)
{
...
int run_64(int fd, t_fileinfo64 fileh, t_lst *root)
{
	...
	if (i < (size_t)fileh.e_shnum)
	{
		...
		if (section.sh_type == SHT_SYMTAB)
		{
			...
			parse_table_64(root, symtable, strtable, section.sh_size / sizeof(t_Elf64_Sym), strtab, sections);
			free(symtable);
			if (!root->next) {
				free(strtable);
				goto cleanup;
